...
def _balanced_binary_decision(
    ...
    """
    Keep decisions in a balanced zone:
    - approve only when confidence is clearly high
    - reject only when confidence is clearly low
    - in the middle uncertainty band, defer to model vote
    """
...
def _resolve_thresholds() -> tuple[float, float]:
    """
    Parse thresholds safely from env without allowing startup/runtime crashes.
    Falls back to defaults if invalid or out of a sensible range.
    """
    try:
        positive = float(os.getenv("HIRING_POSITIVE_THRESHOLD", str(DEFAULT_POSITIVE_THRESHOLD)))
        negative = float(os.getenv("HIRING_NEGATIVE_THRESHOLD", str(DEFAULT_NEGATIVE_THRESHOLD)))
    except ValueError:
        logger.warning(
            "Invalid hiring threshold env vars; using defaults "
            "(HIRING_POSITIVE_THRESHOLD=0.55, HIRING_NEGATIVE_THRESHOLD=0.45)."
        )
        return DEFAULT_POSITIVE_THRESHOLD, DEFAULT_NEGATIVE_THRESHOLD

    if not (0.0 <= negative <= positive <= 1.0):
        logger.warning(
            "Inconsistent hiring thresholds detected; using defaults. "
